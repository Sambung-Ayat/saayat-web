...
function PracticeContent() {
  ...
  const [activeDragItem, setActiveDragItem] = useState<QuestionOption | null>(
    null,
  );
  ...
  const playSound = (type: "correct" | "wrong" | "completed") => {
    const audio = new Audio(
      `/sfx/${type === "correct" ? "correct-answer" : type === "wrong" ? "wrong-answer" : "completed"}.mp3`,
    );
    audio.volume = 0.5;
        ...
        <div className="text-center space-y-8 animate-in zoom-in duration-500 max-w-md w-full my-auto">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              {t.completed} {sessionLimit} {t.questions}.
            </p>
          </div>
    ...
    <div className="min-h-screen flex flex-col items-center bg-background text-foreground pt-20 pb-4 px-4 sm:p-6 transition-colors duration-500 overflow-x-hidden overflow-y-auto">
      ...
      >
        {isStarting && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="flex flex-col items-center justify-center gap-8">
              ...
              <p className="text-muted-foreground animate-pulse text-xl sm:text-2xl tracking-[0.5em] uppercase font-light text-center px-4">
                Bersiap...
              </p>
            </div>
          </div>
        ...
        <main className="w-full max-w-xl flex flex-col items-center space-y-6 sm:space-y-12 pb-20 px-4 my-auto">
          <div
            className={`text-center space-y-4 sm:space-y-6 w-full transition-opacity duration-500 ${!question ? "opacity-0" : "opacity-100"}`}
          >
            ...
            <div className="relative py-2 space-y-4">
              <h1
                className="text-2xl sm:text-3xl md:text-4xl font-arabic leading-[1.8] sm:leading-loose md:leading-[2.2] text-foreground text-center px-2"
                dir="rtl"
              >
            ...
            <p className="text-sm text-muted-foreground/60 font-medium">
              {t.surah}{" "}
              {question?.currentAyah.surahEnglishName ||
                question?.currentAyah.surah}{" "}
              • {t.ayah} {question?.currentAyah.ayah}
            </p>
          </div>
          ...
          <div className="w-full min-h-25 flex flex-col items-center justify-center space-y-4">
            {isSubmitted && (
